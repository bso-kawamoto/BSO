...
- `projects`: 案件。案件名、カテゴリ、期日、アーカイブ状態を管理します。
- `operation_tasks`: タスク。案件ID、親タスクID、階層、担当者、ステータス、カテゴリ、優先度、期日、並び順を管理します。新規案件には標準中タスクを自動登録します。
- `operation_tasks.memo`: タスクごとの進捗メモ。確認中の相手、次にやること、直近の状況を残します。
- 標準中タスクは `企画`、`広報`、`顧客対応`、`運営`、`管理`、`製作`、`調整`、`システム`、`当日対応`、`振り返り` です。自由入力も可能です。
- 案件カテゴリとタスクカテゴリは `大会`、`ALLJAPAN`、`スクール`、`物販`、`チーム`、`イベント`、`広報`、`システム`、`管理部` です。物販、大会、チームカテゴリは該当チーム内で共有表示されます。
...
- `regular_tasks`: 担当者ごとの毎日見るレギュラー業務。通常タスクを毎日増やさず、社員別ビューと今日画面に表示します。
- `project_regular_tasks`: 案件ごとに毎週繰り返す定例業務。通常タスクとして消化せず、案件詳細で管理します。
- `project_regular_task_checks`: 案件定例業務の週次チェック履歴。今週分を実施済みかどうか記録します。
...
- `/projects/[id]`: 大会案件で毎回使う標準小タスクをテンプレートから一括追加できます。既存タスクは重複追加しません。
- `/projects/[id]`: 継続案件向けに、今週分だけチェックできる案件定例業務を管理できます。
- `/today`: 今日の予定、期限切れ、今日期限、3日以内のタスク
- `/employees`: 社員別の未完了タスク、期限切れ、近日期限、予定
- `/six-tournament-deadlines`: 6大会の申込締切日、抽選日を全員で確認できる一覧
- `/admin`: 社長・河本向けの管理画面
- `/admin/deadlines`: 6大会期日の申込締切日、抽選日を上書き管理する画面
...
- 案件カテゴリを使うため、`supabase/migrations/018_add_project_category.sql` をSupabase SQL Editorで適用してください。
- 案件ごとの週次定例業務を使うため、`supabase/migrations/019_add_project_regular_tasks.sql` をSupabase SQL Editorで適用してください。
- 6大会期日の締切延長管理を使うため、`supabase/migrations/020_add_six_tournament_deadline_overrides.sql` をSupabase SQL Editorで適用してください。
